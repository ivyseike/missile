...
struct Player{
    Player(string name_ = "", int tanks_= initial_tanks, int sm_ = initial_sm): name(name_), //fault1, default player name empty
            tanks(tanks_), sm(sm_) {
...
int main(int argc, char *argv[]) {
    ...
    int player = 1 ;
    int counter = 0;
    while(players[0].tanks && players[1].tanks){
        turn_count++;
        ...
        if(command == "stats" || command == "help" || command == "reset"){
            ...
            if(split_list.size() && split_list[0] == "RM" || split_list[0] == "SM"){
                if(split_list[0] == "RM"){
                    players[player].targeted.push_back(split_list[1]); // fault 4, not veryfing validity and adding to targeted list
                            ...
                            if(players[player].sm){
                                ...
                                if(verifyCoordinate(newC))
                                   ...
                                   if(players[counter].board[newC[1]-'1'][newC[0]-'A'] == 1){
                                       ...
                                       if(players[0].destroyed.size() == 0 && players[1].destroyed.size() == 0){
                                           players[player].sm++;
                                       }// fault 3, not adding extra sm when used sm to destroy
										*/
